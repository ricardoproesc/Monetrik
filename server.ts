...
// ─── Envio do e-mail de verificação (HTML Monetrik via Resend) ────────────────
app.post("/api/auth/send-verification", async (req, res) => {
  try {
    ...
    res.status(status).json({ error: err?.message || "Falha ao enviar e-mail de verificação" });
  }
});
...
// 6. Migration: Download modelo estático (aba Instruções + Lançamentos)
app.get("/api/migration/template", (req, res) => {
  try {
